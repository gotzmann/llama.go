...
static bool kv_cache_init(
    ...
    printf("\n\n >>> kv_cache_init | n_embd = %d | n_layer = %d | n_mem = %d | n_elements = %d <<<\n\n", 
    n_embd, n_layer, n_mem, n_elements);
    ...
    //printf("\n\n===kv_cache_init | wtype = %d | n_elements = %d\n\n", wtype, n_elements);
...
void printTensor(struct ggml_tensor *tensor, char* name) {
    char *type = "";
    if (tensor->type == GGML_TYPE_F16) type = "FP16";
    if (tensor->type == GGML_TYPE_F32) type = "FP32";
    if (tensor->type == GGML_TYPE_Q4_0) type = "INT4";

    printf("\n\n=== [ %s | %s | %d:%d:%d ] ===\n", 
        name, type, tensor->ne[0], tensor->ne[1], tensor->ne[2]); // DEBUG

    for (int nn = 0; nn < std::min(12, tensor->ne[1]); nn++) {
        printf("\n %d x %d ...\t", nn, tensor->ne[0]);
        for (int ii = 0; ii < std::min(12, tensor->ne[0]); ii++) {
            if (tensor->type == GGML_TYPE_F32) {
                float f = * (( (float * )(tensor->data)) + nn*tensor->ne[0] + ii);
                printf("%.3f\t", f);
            }    
            if (tensor->type == GGML_TYPE_F16) {
                float f = GGML_COMPUTE_FP16_TO_FP32(* (((ggml_fp16_t*)(tensor->data)) + nn*tensor->ne[0] + ii) );
                printf("%.3f\t", f);
            } 
        }
    }
...
struct llama_tokenizer {
    ...
    void tokenize(const std::string & text, std::vector<llama_vocab::id> & output) {
        ...
        for (size_t i = 1; i < symbols_.size(); ++i) {
            ...
            //printf(" [ sym[%d] = %s ] ", int(i), symbols_[i].text); // DEBUG
...
private:
    void try_add_bigram(int left, int right) {
        ...
        //printf("\n** symbols[left].Text = %s | N = %d ** ", symbols_[left].text, symbols_[left].n);
	    //printf("\n** symbols[right].Text = %s | N = %d ** ", symbols_[right].text, symbols_[right].n);
        ...
        //printf(" [ text = %s | token id = %d ] ", text.c_str(), (*token).second ); // DEBUG
        ...
        //printf(" [ token score = %f | len = %d ] ", tok_score.score, text.size() ); // DEBUG
...
static llama_vocab::id llama_sample_top_p_top_k(
    ...
    printf("\nn_logits = %d", n_logits); // DEBUG
    printf("\nlogits.size() = %d\n", logits.size()); // DEBUG
    for (int ii = 0; ii < 10; ii++) {
        printf("| logits[%d] = %f |", ii, logits[ii]);
    }
    ...
    printf("\nllama_sample_top_p_top_k = %d", logits_id[idx].second); // DEBUG

...
static llama_vocab::id llama_sample_top_p_top_k(
    ...
    //printf("\n=== logits.size() = %d", logits.size()); // 32,000
    //printf("\n=== n_logits = %d", n_logits); // 32,000
    printf("\n=== logits | %d ===\n", n_logits);
    for (int i = 0; i < 8; i++) {
        printf("%.4f ", logits[i]);
    }
    printf(" ... ");
    for (int i = logits.size()-1; i >= logits.size()-8; i--) {
        printf("%.4f ", logits[i]);
    }
    printf("\n=== last_n_tokens | %d ===\n", last_n_tokens.size());
    for (int i = 0; i < last_n_tokens.size(); i++) {
        printf("%d ", last_n_tokens[i]);
    }
...
/*
    printf("\n=== logitsID BEFORE | %d ===\n", logits_id.size());
    for (int i = 0; i < 6; i++) {
        printf("{ %.3f | %d } ", logits_id[i].first, logits_id[i].second);
    }
    printf(" ... ");
    for (int i = logits_id.size()-6; i < logits_id.size()-1; i++) {
        printf("{ %.3f | %d } ", logits_id[i].first, logits_id[i].second);
    }
*/
    {
    ...
    printf("\n=== logitsID AFTER | %d ===\n", logits_id.size());
    for (int i = 0; i < 6 /*std::min(8, logits_id.size())*/; i++) {
        printf("{ %.3f | %d } ", logits_id[i].first, logits_id[i].second);
    }
    printf(" ... ");
    for (int i = logits_id.size()-6; i < logits_id.size()-1; i++) {
    ...
    //// --- INLINED: sample_top_k(logits_id, top_k);
    ...
    printf(" ... ");
    for (int i = probs.size()-6; i < probs.size()-1; i++) {
        printf("%.3f  ", probs[i]);
    }
    ...
    printf("\nidx = %d", idx);
    printf("\nlogit_id = %d | logit weight = %f", logits_id[idx].second, logits_id[idx].first);
...
void printTensor(struct ggml_tensor *tensor, char* name) {
    ...
    for (int nn = 0; nn < std::min(int64_t(12), tensor->ne[1]); nn++) {
        printf("\n %d x %d ...\t", nn, tensor->ne[0]);
        for (int ii = 0; ii < std::min(int64_t(12), tensor->ne[0]); ii++) {
